...
def analyze_numerical_distribution(df, column):
    ...
    # KDE by Label with mean and std lines
    plt.figure(figsize=(10, 6))
    for label in df['Label'].unique():
        subset = df[df['Label'] == label][column].dropna()
        ...
        # Mean line
        idx_mean = np.argmin(np.abs(x_kde - mean_val))
        y_at_mean = y_kde[idx_mean]
        plt.vlines(
            x=mean_val,
            ymin=0,
            ymax=y_at_mean,
        ...
        for bound in [mean_val - sd_val, mean_val + sd_val]:
            idx_sd = np.argmin(np.abs(x_kde - bound))
            y_at_sd = y_kde[idx_sd]
            plt.vlines(
                x=bound,
                ymin=0,
                ymax=y_at_sd,
    ...
    plt.title(f"Distribution of {short_title} by Food Type")
    plt.xlabel(short_title)

    # Create custom legend entries
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], color='gray', linestyle='-', label='KDE'),
        Line2D([0], [0], color='gray', linestyle='--', label='Mean'),
        Line2D([0], [0], color='gray', linestyle=':', label='±1 SD')
    ]
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/{simple_name}_kde_by_label.png")
    plt.close()
